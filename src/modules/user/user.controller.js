...
// generate token
const token = jwt.sign({id:newUser._id},process.env.JWT_SECRET_CREATE_USER,{expiresIn: "1d"});
...
export const confirmEmail=async (req,res,next)=>{
    ...
    const {id}  = jwt.verify(token,process.env.JWT_SECRET_CREATE_USER)
    const User = await userModel.findByIdAndUpdate(id,{isConfirmed:true},{new:true}).select("-password")
    if(!User) 
...
export const signIn = async (req, res, next) => {
    ...
    const token = jwt.sign({ id: _user._id }, process.env.JWT_SECRET_LOGIN_USER, { expiresIn: "1d" });
